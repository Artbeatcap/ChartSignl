...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  refreshSubscription: async () => {
    ...
    try {
      ...
      // Sync to backend (updates Supabase subscriptions table)
      await subscriptionService.syncToBackend(user.id);
    } catch (error) {
      console.error('Error refreshing subscription:', error);
      // Fallback: try to get status without syncing
      try {
        const status = await subscriptionService.getSubscriptionStatus(user.id);
        set({ isPremium: status.isActive });
      } catch (checkError) {
  ...
  signOut: async (): Promise<boolean> => {
    try {
      // Log out from subscription service (handles RevenueCat on mobile)
